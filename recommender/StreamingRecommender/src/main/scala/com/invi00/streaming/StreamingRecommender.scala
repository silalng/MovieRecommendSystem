...
object ConnHelper extends Serializable{
  lazy val jedis = new Jedis("sparkRecommend")
  lazy val mongoClient =
    MongoClient(MongoClientURI("mongodb://sparkRecommend:27017/recommender"))
}
...
object StreamingRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //创建到Kafka的连接
    val kafkaPara = Map(
      "bootstrap.servers" -> "sparkRecommend:9092",
  ...
  def getTopSimMovies(num:Int, mid:Int, uid:Int,
    ...
    allSimMovies.filter(x =>
      !ratingExist.contains(x._1)).sortWith(_._2 > _._2).take(num).map(x => x._1)
  }
  ...
  def saveRecsToMongoDB(uid:Int,streamRecs:Array[(Int,Double)])
                       (implicit mongConfig: MongConfig): Unit ={
    //到StreamRecs的连接
